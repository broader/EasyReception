...
# Page Functions
def index():
     divs = [DIV( _header(), **{'id' : HEADERDIV, 'style':'padding:25px 2em 1.5em 20px;'}), DIV(_nav(), **{'id' : NAVDIV})]
     print Sum(divs)
     script = \
     '''
...
def _loginScript( ):
     ...
     # add registration action page
     paras.append('registration/registration.ks/index')
     
     # add title for dialog 
     paras.append( "<h4 style='font-weight:bold;'>%s</h4>"%_('Registration Process'))
     
     paras = tuple(paras)
     script = '''
     var buttons=$("%s").getElements("a");
     var mainDiv="%s";
     // popup dialog plugin
     var dlgId="%s",mavdlgCss="%s", mavdlgJs="%s";
     var mavdigUrl="%s", title="%s";
...
def _header(**args):
	  ...
	  contents = [  DIV( SPAN( page_loginSpan(), **{'id' : LOGINSPAN } ), **{'id' : 'topnav'}), \
		             H1(  _('EasyReception - A Congress Management Portal'),style='font-size:2.2em;'), \
		             SPAN( _("Simple, Professional and Methodical"),\
		             		 style='margin-left:0.5em;font-weight:bold;font-style:italic;text-decoration:underline;')]
