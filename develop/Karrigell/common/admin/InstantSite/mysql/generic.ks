...
def index(start=0):
    ...
    lines = TR(Sum([TH(f) for f in table.fields])+TD("&nbsp;")*2)
    ...
    for r in recs:
        ...
        for fname,info in field_info:
            cells += [TD(A(r[fname],Class="edit",
                href="edit?rec_id=%s&fname=%s" %(r[id_field],fname)))]
        lines += TR(Sum(cells))
...
def edit(rec_id,fname=None):
    ...
    r = table[int(rec_id)]
    print FORM(INPUT(Type="hidden",name=id_field,value=r[id_field])+
        TABLE(Sum(_cells(r)))+
...
def new():
    ...
    title = H2("New record")
    hidden = INPUT(name=id_field,Type="hidden",value=-1)
