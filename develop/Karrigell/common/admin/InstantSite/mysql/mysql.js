...
keys = new Array('no','UNIQUE','PRIMARY KEY')
...
function default_timestamp(elt)
{
    if (elt.value=='') {elt.value='CURRENT_TIMESTAMP'}
    else { elt.value=''}
    validate()
}
...
function change_type()
{
    ...
    if (selType == 'TIMESTAMP')
    {
      radio = '<INPUT Type="checkbox" name="default" value=""'
      radio += ' onClick="default_timestamp(this)">'
      radio += 'current timestamp'
      document.getElementById('default_cell').innerHTML=radio;
    }
    else
    {
      entry = '<INPUT name="default" onKeyUp="validate()" onChange="validate()">'
      document.getElementById('default_cell').innerHTML=entry;
    }
...
function validate()
{
 ...
 { 
   if (selType == 'TIMESTAMP' && elt.value != '')

   { ch += ' DEFAULT '+elt.value+' ' }
   else if (is_unsigned[selType] == undefined)
   { ch += ' DEFAULT &quot;'+elt.value+'&quot; ' }
   else
   { ch += ' DEFAULT '+elt.value+' ' }
 }
