...
def _get_cursor():
    connection = sqlite.connect(Session().path)
...
def drop_table_confirm():
    connection,cursor = _get_cursor()
    cursor.execute('DROP TABLE %s' %Session().table)
    del Session().table
    raise HTTP_REDIRECTION,"view"
...
def generate_script_2(security,login=None,passwd=None):
    ...
    db_sqlite = Import("db_sqlite.py")
    for key in "inits","open_","all_records",\
        "select_by_id","insert_or_update","remove":
            locals()[key] = getattr(db_sqlite,key)
