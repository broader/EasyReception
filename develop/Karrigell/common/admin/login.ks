...
def login(valid_in,redir_to,role="admin",add_user=0):
    # check if users database initialized
    if not k_users_db.exists(CONFIG):
        print "Users database doesn't exist. You must "
        print A("set admin login and password",href="../set_admin.ks?url=%s" %redir_to)
        raise SCRIPT_END
    ...
    f += INPUT(Type="hidden",name="add_user",value=add_user)
...
def check_login(valid_in,redir_to,login,passwd,role="admin",add_user=0):
    ...
    if admin and (not role.strip() or admin[0]["role"] in role.split(",")):
        ...
        # if add_user is set and user doesn't exist, create it
        key = _make_session_key()
        if db(login=login):
            print _("Wrong password")
            raise SCRIPT_END
        recid = db.insert(login=login,password=passwd,role="edit",
            session_key=key,last_visit=datetime.datetime.now(),
            nb_visits=0)
        db.commit()
        r = db[recid]
    else:
