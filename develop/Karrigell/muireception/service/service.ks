...
def _showServiceJs(category):
	paras = [APP, CONTAINERID(category)]
	...
	var options = {
		onload:function(){    		
			treeTable = new TreeTable( 
				container,				
				{colsModelUrl:colsModel,dataUrl:rowsUrl}
	...
	// inert action buttons to each row in the table
	treeTable.tableInstance.element
	.getElements('tr').slice(1)
	.each(function(row){		
		td = row.getLast();
