var ImageZoom = new Class({
	...
	initialize: function(option){
		...
		this.imageUrl = $(this.options.thumbContainer).getElement('a');
		this.thumb_image = this.imageUrl.getElement('img');
	...
	generateZoomer: function( thumb_size ){
		...
		this.bigImage = new Asset.image( this.imageUrl.get('href'), {
			id:this.options.zoomerImage,
			onload: function(){				
				...
				new Element('div', {
					id: this.options.thumbZoomerRegion,
					styles: {
						'width': regionWidth,
						'height': regionHeight,
						'opacity': .7
					}
				}).injectInside(this.options.thumbContainer);
				/* move the zoomed image when the zoomer region is dragged on the thumbnail */
				new Drag(this.options.thumbZoomerRegion, {
					...
					onDrag: function(el){
						...
						this.setPosition(this.options.zoomerImage,left,top);
				...
				this.DragBig = new Drag(this.options.zoomerImage,{
					modifiers: {x:'left',y:'top'},
					grid:1,
					onDrag: function(elem){
						...
						{							
							...
							//alert("pos.x:"+(-pos.x)+",bigImage.width:"+this.bigImage.width+",thumb_size.x:"+thumb_size.x+",this.zoomSize:"+this.zoomSize);
							
							this.setPosition(this.options.zoomerImage,left,top);					
						};
						/* moves the zoomed region on thumbnail according to the position of the zoomed image */
						this.setPosition(this.options.thumbZoomerRegion,-(left/ratioX),-(top/ratioY));
