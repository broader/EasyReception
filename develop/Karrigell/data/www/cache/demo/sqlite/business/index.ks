['index', 'browse', 'song_list', 'details', 'handle_cart', 'clear_session']
...
def browse(key):
    ...
    db = Import("songDb",REL=REL,CWD=CWD).db
    ...
    for elt in elts:
        browse += DD(A(elt[1],
            href="song_list?key=%s&value=%s" %(key,elt[0])))
...
def song_list(**kw): #key="",value=""):
    ...
    for s in songs:
        lines += TR(TD(A(s['breton'],
            href="details?song=%s" %s['__id__']))
