['isLinkName', 'formatWikiname', 'getWikiwords', 'test']
...
wikiLink=re.compile(r"\b[A-Z][a-z]+_?[A-Z][a-z]+.*?\b")
...
def getWikiwords(name):
    return re.sub(r'([a-z_])([A-Z])', r'\1|\2', name).replace('_','').split('|')

class BuanDoc:

    def __init__(self, name, text):
        self.wikiNames = set() # new pages (words as valid Wiki names)
        self.pageName = name
        self.text = cgi.escape(text)
        self.listqueue = []
        self.intable = False
        self.make_html()
        # close any open tables or lists
        if self.intable:
            self.text +=  "\n</table>\n"
        else:
            while self.listqueue:
                self.text += ' ' * (len(self.listqueue)-1) + '</%s>\n' % self.listqueue.pop()
    ...
    def make_html(self):
        ...
        self.ol_depth = 0
        self.ul_depth = 0
        res = line.sub(self.handle_line,self.text)
        while self.listqueue:
    ...
    def handle_wikiLink(self,mo):
        ...
        return '<a href="../show/%s">%s</a>' % (link,link)
    ...
    def handle_anchor(self,mo):
        txt,href = mo.groups()
        if href.startswith('<a href="../show/'):
