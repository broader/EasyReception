['page_loginForm', 'page_accountValid']
"""
...
def _loginFormJs():
	...
	paras.append('portaLayout.ks/page_accountValid')

	paras.extend([ url for url in ( 'lib/formcheck/theme/red/formcheck.css', 'lib/formcheck/formcheck.js', 'lib/formcheck/lang.js.pih')])
	...
	function closeLoginPanel(){
		...
		loginBn.getPrevious().show();
		loginBn.show();

		// delete login panel
		$('loginPanel').retrieve('form').dispose();
	...
	loginFormchk = new FormCheck( loginForm,{
		...
		// constructs the url to portal page
		var url = document.URL.split('/');
		url.pop();
		url.push('portal.pih');
		url = url.join('/');
		url = new URI(url);
		if(Browser.Engine.trident) window.open(url.toString(), "_blank", "fullscreen=1,");
		else{
	...
	*/
	...
	window.addEvent('domready', function() {
	    var inputElements = $(loginForm).getElements('input');

	    // add callback function for changing different user role
		...
		inputElements.slice(0,2).each(function(input){
	    ...
	    // submit button
	    inputElements[2].addEvent('click',function(e){
		loginFormchk.onSubmit(e);
	    });
	    // cancel button
	    inputElements[3].addEvent('click',function(e){
		new Event(e).stop();
		closeLoginPanel();
	    });
