...
# import the config file by searching recursive directories
isRightDir, levels = False, 0
while not isRightDir:
    path = ['..' for i in range(levels)]
    path.append('config')

    try:
	INITCONFIG = Import( '/'.join(path), rootdir=CONFIG.root_dir)
	isRightDir = True
    except:
	levels += 1
