['page_editDialog', 'page_serviceInfo', 'page_editForm', 'page_reserveEdit', 'page_reservationList', 'index']
import copy
...
def _registrationList( ):
	...
	caption = CAPTION( _('Your travel Reservations'), **attrs)
	...
	tbody = []
	if data :
		for pserial, travel in data.items() :
			# For distinguishing from travel list tree table,
			# here we get parent service id
			pid = model.serial2id(pserial)
			for i, room in enumerate(travel) :
				if i == 0 :
					sid = '-'.join(('node', pid))
				else:
				...
				# render the travel row and all types of rooms of the travel
	...
	$(document).ready(function(){
		var listDiv="#%s", parentNode=".%s", action="%s", dialogUrl="%s";
	...
	'''%( RESERVEDDIV, SUBCATEGORY_TAG, 'edit' , '/'.join((APPATH, 'page_editDialog')) )
	table.append(pagefn.script(src, link=False))
	return Sum(table)

EDITFORM = 'reserveEdit'
SERVICEINFO = 'serviceInfo'
def page_editDialog(**args):
...
def page_serviceInfo(**args):
	...
	# get travel name
	travelSerial = values.get('parent')
	pValues = _getService(model.serial2id(travelSerial), props=('detail',))
	travel = pValues.get('alias')
	caption = CAPTION( travel, **attrs)
	# render information
	tbody = []
...
def _sortTravels(data):
	sorted = {}
	for travel in data :
		values = dict(zip(PROPS4TABLE, travel))
...
CATEGORYNAME = 'Travel'
def _getTravels( ):
	''' Get all the travel service items from database.
	'''
	fnArgs = ('category', )
	fn = (lambda c : c == CATEGORYNAME)
	travels = model.filterByFunction(so.user, 'service', PROPS4TABLE, fn, fnArgs)
	if travels :
		travels = _sortTravels(travels)
	return travels

PROPS4TABLE =  ['serial', 'detail', 'description', 'price', 'amount']
TITLES = [ _('Serial'), _('Item'), _('Description'), _('Unit Price'), _('Amount')]
...
def _travelList( ):
	''' Get all a list for all capable travel.
	'''
	data = _getTravels()
	# the caption for the table
	attrs = {'style' : 'text-align: left; font-size: 1.8em;font-weight:bold;' }
	caption = CAPTION( _('Travel List'), **attrs)
...
TRAVELISTDIV = 'travelist'
def _topDiv( ):
	content = [ _travelList(),]
	attr = copy.copy(PAGEATTR)
	#attr['style'] += 'border-bottom: 1px solid #8B8378;'
	attr['id'] =TRAVELISTDIV
	PRINT( DIV(Sum(content), **attr))
