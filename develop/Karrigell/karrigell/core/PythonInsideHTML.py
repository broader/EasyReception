...
class PIH:
    ...
    def __init__(self,fileName=None,defaultEncoding=None):
    ...
    def parse(self,fileObject):
        ...
        self.destLine=0 # first line with an origin in pih code
        while self.pointer<len(self.pihCode):
            ...
            if rest.startswith("<indent>"):
                ...
                if tag=="<%=":
                    ...
                    self.output.write('STDOUT(str(')
                else:
                    self.output.write('STDOUT(_(')
                ...
                if pythonCode.strip().lower()=="end":
                    ...
                    for i in range(len(pythonCodeLines)):
                        ...
                        if self.indentation=="on":
                            ...
                            line=" "*(j-self.indent1)+line.strip()
                        self.output.write(" "*self.indent)
    ...
    def flushHTML(self):
        ...
        if html:
            ...
            p = self.startHTML
            line_num = self.getLineNum(p)
            indent = self.getLineIndent(p)
                ...
                if htmlLine.strip():
                    ...
                    if out:
                        ...
                        if self.indent != indent:
                            ...
                            self.output.write('STDOUT("%s")\n' %output)
                            indent = self.indent
                            output = out
                            self.lineMapping[self.destLine]=line_num
                            self.destLine+=1
                            line_num = self.getLineNum(p)
                        else:
                            output += out
    ...
    def pythonCode(self,default_encoding=None):
